...
public class mainPageController extends Application{
	...
	private void initialize() {
	 ...
	 // Set up Results table columns
	    AddressClmReportTable.setCellValueFactory(cellData -> 
	        new SimpleStringProperty(cellData.getValue().getAddress()));
	            
	    NumOfAbusesClmReportTable.setCellValueFactory(cellData ->
	        new SimpleStringProperty(String.valueOf(cellData.getValue().getAbuseCount())));
	            
	    LinkClimReportTable.setCellValueFactory(cellData ->
	        new SimpleStringProperty(cellData.getValue().getLink()));
	}
